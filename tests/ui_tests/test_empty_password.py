...
# Testing that pressing the login button shows an error if the password field is empty.
@pytest.mark.usefixtures("custom_login")
def test_empty_password(is_visible, create_wait):
    try:
        wait = create_wait
        ...
        error_message_element_under_field = wait.until(
            is_visible(By.XPATH, "//div[@id='password-container']/p/span[@id='uniName_947Help']")
        )
        ...
        assert error_message_element.is_displayed(), "Empty password test failed (no error message)."
